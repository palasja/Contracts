...
namespace PayrollBLL.Reader
{
    class PayrollReader
    {
        ...
        public PayrollDTO ReadPayroll(DataTable dt)
        {
            PayrollDTO payroll = ReadPayrollPayment(dt);
        ...
        public async Task<PayrollDTO> ReadPayrollAsync(DataTable dt)
        {
            PayrollDTO payroll = await Task.Run(() => ReadPayrollPayment(dt));
            payroll.Period = ReadPeriod(dt.Rows[0][0]);
            payroll.Worker = ReadFullName(dt.Rows[2][0]);
        ...
        private PayrollDTO ReadPayrollPayment(DataTable dt)
        {
            ...
            for (int i = startPaymentInfo; i < dt.Rows.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        PaymentDTO payment1 = ReadPayment(row.ItemArray, type);
                        payroll.Payments.Add(payment1);
                    }
                    if (!String.IsNullOrEmpty(row[8].ToString()))
                    {
                        PaymentDTO payment2 = ReadPayment1(row.ItemArray, type1);
        ...
        /// <returns></returns>
        private PaymentDTO ReadPayment(object[] cels, int type)
        {
        ...
        /// <param name="cels"></param>
        /// <returns></returns>
        private PaymentDTO ReadPayment1(object[] cels, int type)
        {
            PaymentDTO payment1 = new PaymentDTO();
        ...
        private void ItogOnType(PayrollDTO payroll, string strType, string strSumm)
        {
